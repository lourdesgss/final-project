...
\begin{itemize}
    ...
    \item \textbf{Camera HTTP visual stream} —  TODO
    \item \textbf{MQTT CXV} - TODO
    \item \textbf{Festo Controllers} - TODO
\end{itemize}

As a result, the inventory and AS-IS architecture in this report should be read
as a best-effort reconstruction based on the captures we did obtain,
supplemented by vendor documentation and protocol specifications. A follow-up
assessment with access to a fully operational production run would be needed to
confirm completeness of the communications map and to rule out any device or
flow not visible during our limited window.
TODO: rewrite less AI-like, simpler words but still proffesionall.

\subsection{General Reflection}
Working with a physical lab environment introduced constraints that a purely
desk-based security assessment would not face. Access to the FESTO plant was
time-boxed and shared with other coursework groups, which limited how much we
could experiment with capture conditions or repeat captures under different
operating states. This is a realistic constraint in any OT environment, however
— production systems are rarely available on demand for security assessment
either, and our experience reflects a smaller-scale version of the access
negotiation that real OT security assessments require.

The exercise also reinforced a recurring theme in OT security: the gap between
an idealised Purdue-model architecture and what is actually deployed in
practice. Several of the flows we did capture did not cleanly map to the zone
boundaries we expected going in, which shaped both our threat modelling and
several of the security requirements in Section~\ref{sec:secreq} (notably the
explicit firewall conduits in SR-01 and SR-02). We consider this a useful
finding in its own right: it suggests that segmentation work on a system like
this needs to start from observed traffic rather than assumed architecture.
TODO: rewrite less AI-like, simpler words but still proffesionall - shorter
sentences.
