\section{Trust Zones and Segmentation}
...
\subsubsection{IEC 62443 Violations}
...
\subsubsection{ISO/IEC 27001 Violations}
...
\subsubsection{NIS2 Directive Violations}
...
\subsubsection{Defense-in-Depth Violations}
\begin{table}[H]
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{@{}p{0.55\textwidth}p{0.4\textwidth}@{}}
\toprule
...
\subsubsection{Least Privilege \& Zero Trust Violations}
...
\subsubsection{F-01 - CRITICAL: Fully Flat /18 Network}
...
\bottomrule
\end{tabular}
\caption{Flat network architecture allowing unrestricted communication}
\end{table}

\subsubsection{F-02 - CRITICAL: Unrestricted PLC Access}
...
\bottomrule
\end{tabular}
\caption{Unrestricted access to PLC services from any host on the OT subnet}
\end{table}

\subsubsection{F-03 - CRITICAL: No MES / OT Separation}
...
\bottomrule
\end{tabular}
\caption{Direct communication paths between MES and OT systems without network separation}
\end{table}

\subsubsection{F-04 - CRITICAL: Unregistered Device Programming PLCs}
...
\bottomrule
\end{tabular}
\caption{Undocumented device maintaining persistent programming access to PLC S2}
\end{table}

\subsubsection{F-05 - HIGH: PLC HTTP Web Servers (Plaintext)}
...
\bottomrule
\end{tabular}
\caption{PLC web services exposed over unauthenticated plaintext HTTP}
\end{table}

\subsubsection{F-06 - HIGH: OPC-UA Accessible Network-Wide}
...
\bottomrule
\end{tabular}
\caption{Network-wide accessibility of OPC-UA services without verified security controls}
\end{table}

\subsubsection{F-07 - HIGH: TP-Link Access Point on OT Subnet}
...
\bottomrule
\end{tabular}
\caption{TP-Link access point present on the operational technology network}
\end{table}

\subsubsection{F-08 - HIGH: Visitor/Unvetted Laptop on OT Network}
...
\bottomrule
\end{tabular}
\caption{Visitor laptop connected directly to the live OT environment}
\end{table}

\subsubsection{F-09 - HIGH: VMware Host with Unknown VM Roles}
...
\bottomrule
\end{tabular}
\caption{VMware host operating on the OT subnet with undocumented virtual machines}
\end{table}

\subsubsection{F-10 through F-15 - Medium / Low Findings}
...
\subsection{Standards Alignment Summary}
