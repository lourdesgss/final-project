...
\begin{longtable}{L{2.1cm} L{2cm} L{1.7cm} L{2.6cm} L{2.8cm}}
\toprule
\textbf{Source} & \textbf{Destination} & \textbf{Protocol/Port} & \textbf{Direction} & \textbf{Purpose} \\
\midrule \endhead
MES & PLC & OPC-UA / 4840 & MES (client) $\to$ PLC (server) & Read/write process data \\
HMI / PLC & MES & NTP / 123 & Clients $\to$ NTP server & Time sync \\
Internet & MES & HTTPS / 443 & Internet $\to$ MES & Webshop orders \\
Cobot-ML PC & Camera & HTTP / 80 & Cobot-ML $\to$ Camera & Fetch inspection image \\
Cobot-ML PC & PLC & OPC-UA / 4840 & Cobot-ML $\to$ PLC & Process data from image \\
PLC & IO Device & PROFINET (L2) & PLC $\to$ IO & Cyclic I/O exchange \\
Festo Ctrl & CPX-IoT (CVX) & MQTT / 1883 & Ctrl $\to$ CVX & Publish aggregated data \\
RFID Reader & PLC & PROFINET (L2) & Reader $\to$ PLC & RFID identification \\
PLC & Robot Arm & PROFINET (L2) & PLC $\to$ Robot & Motion commands / status \\
HMI & PLC & S7comm+ / 102 & HMI $\to$ PLC & Read/write panel tags \\
MES PC & PLC & S7comm+ / 102 & MES $\to$ PLC & PLC programming \\
MES PC & HMI & S7comm+ / 102 & MES $\to$ HMI & HMI programming/config \\
Power Monitor & Festo Ctrl & Modbus / 502 & Monitor $\to$ Ctrl & Power consumption data \\
\bottomrule
\end{longtable}
}
