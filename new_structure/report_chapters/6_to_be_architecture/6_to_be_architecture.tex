...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{./report_chapters/6_to_be_architecture/img/logic_redesign.pdf}
...
\subsubsection{IDMZ}
The IDMZ acts as a buffer between IT and OT devices. No connection is allowed 
to pass from the IT to OT and vice versa. Every traffic exchange terminates inside IDMZ,
and its re-initiated to the other side. This solution implements IEC 62443-3-3 SR 5.1 principle.
\subsubsection{OT Cell Zone}
The OT Cell Zone Contains devices supporting production, but not being part of the physical production
process. The Engineering Workstation is placed in this logical zone, however, in physical
implementation, this node will be further isolated into separate VLAN.
\subsubsection{OT Production Zone}
The OT Production Zone contains devices being part of the direct physical production process.
...
least-privilege and IEC 62443-2-1 §4.3.3.
...
In AS-IS architecture, there is no documented backup procedure. The backup node receives
...
inbound traffic from OT. Satisfies IEC 62443-2-1 §4.3.4 and ISO/IEC 27001 A.12.3.
\subsubsection{Log \& Backup proxy}


\subsection{Firewalls}

This section only describes briefly the high-level purpose of the firewall nodes.
Further specification on permitted conduits is described in 7.2.2.
...
The Network Implementation diagram is appended to the report for better visibility.
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{report_chapters/6_to_be_architecture/img/redesign_iml.pdf}
    \caption{Network Implementation}
\end{figure}

\paragraph{VLAN Segmentation \& Restricted Routing}
...
The firewall has one tagged VLAN port for each VLAN - which is a default gateway
for each node within that VLAN.
...
Because the switch never bridges VLANs, the only path between any two zones 
is through the firewall. The firewall applies a default-deny policy: 
traffic between VLANs is dropped unless a rule explicitly permits it. 
Traffic within a VLAN (for example PLC-to-HMI Profinet inside one station) 
is switched locally and never reaches the firewall. 
Profinet RT traffic is Layer 2 only and is confined to its station VLAN; 
it is never routed across the firewall.
This realises the four logical firewall boundaries 
(Edge, IT–IDMZ, IDMZ–OT Cell, OT Cell–OT Production) on a single physical 
firewall, where each boundary corresponds to a rule set 
between the relevant VLAN interfaces. 
It satisfies IEC 62443-3-3 SR 5.1 (Restricted Data Flow) and SR 5.2 (Zone Boundary Protection).
...
\pagebreak
\paragraph{Justified Protocols Routes}
The permitted flows exists to allow a required operational
functionality - all other are denied.
The IT node never reaches an OT node directly. All IT to OT connections are terminated
and re-initiated in the IDMZ proxy to the other side. \\ \vspace{1em}

\renewcommand{\arraystretch}{1.3} % 30% more row height
\begin{tabularx}{\textwidth}{p{2.5cm} X X X}
    \textbf{Flow} & \textbf{Path} & \textbf{Protocol/Port} & \textbf{Justification}  \vspace{0.5em}\\
\toprule
    Process Data, MES to PLC & MES to OPC UA proxy to PLC's  OPC UA server & OPC UA/TCP4840 & MES reads/writes process data. MES connection is terminated in OPCUA proxy. No direct connection between MES and PLC. \\
    \midrule
    Web data, MES to OT nodes & MES to Nginx to OT nodes web server & HTTPS/TCP443 (IT side), HTTP/TCP80 (OT side) & Required Web data from OT nodes is fetched by Nginx reverse proxy instead of MES directly - plain text HTTP stays in OT zone, and Nginx connects to MES through HTTPS. \\
    \midrule
    Telemetry, MQTT CVX device to MES & MQTT CVX to MQTT broker to MES (subscribe) & MQTT/TCP8883 TLS & OT only publish, IT only subscribe\\
    \midrule
    PLC/HMI programming & Engineering Workstation to HMI's/PLC's & S7comm+/TCP102 & Programming only from the dedicated workstation in the engineering VLAN \\
    \midrule
    Cobot process data & Cobot/MLvisionPC to PLC2 & OPC UA/TCP4840 & Vision PC needs PLC2 data only; scoped to Station 2 \\
    \midrule
    Time synchronization & All devices to NTP server & NTP/UDP123 & Single time source for all OT devices \\
    \midrule
    Logging & All devices to Syslog collector & Syslog/UDP514 & Centralised log collection for monitoring \\
    \midrule
    Webshop & Internet clients to MES & HTTPS/TCP443 & Public webshop terminates in IT, no OT reachability \\
\end{tabularx}
